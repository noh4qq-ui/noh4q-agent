...
Blogs: Telegraph + Beehiiv#!/usr/bin/env python3
"""
NOH4Q AGENT - PHASE 4D (Analytics + Auto-Reply + Queue)
All previous features + analytics, auto-reply, and content queue
"""
...
TELEGRAM_GROUP_ID = os.getenv("TELEGRAM_GROUP_ID", "")  # Optional: group chat ID
...
class DB:
    ...
    def get_recent_posts(self, limit=10):
        self.c.execute("SELECT platform, status, ts FROM analytics ORDER BY id DESC LIMIT ?", (limit,))
        return self.c.fetchall()
    ...
    def pop_queue(self):
        ...
        if not row:
            return None
        self.c.execute("UPDATE queue SET status='done', posted_at=? WHERE id=?",
...
def get_commodity_price(commodity="XAU/USD"):
    ...
    if not yf_sym:
        return {"error": f"Unknown {commodity}"}
    try:
                        ...
                        for member in msg["new_chat_members"]:
                            ...
                            requests.post(
                                ...
                                json={"chat_id": chat_id,
                                      "text": f"👋 Welcome {name}! This is the NOH4Q community. Type /start to see what I can do."},
                                timeout=10
                            )
                    if text:
...
def handle_command(text, chat_id):
    ...
    if text == "/start":
        tg_send(f"🤖 NOH4Q Agent v4D\n"
                ...
                f"/post <topic> <type> - post to socials\n\n"
                f"💱 Trading:\n"
                f"/forex - live prices\n"
                f"/trade <asset>\n"
                ...
                f"📊 Analytics:\n"
                f"/analytics - platform performance\n\n"
                f"📝 Queue:\n"
        ...
        if not summary:
            tg_send("📊 No posts tracked yet. Wait for the next cycle.")
            return
        msg = "📊 ANALYTICS\n━━━━━━━━━━━━━━━━━━━\n"
        for s in summary:
            msg += f"\n{s['platform'].upper()}\n"
        ...
        if not topic:
            ...
            tg_send(f"✅ Added to queue: '{topic}'")
        ...
        if not items:
            ...
            for i, item in enumerate(items[:20], 1):
                msg += f"  {i}. {item[1]} ({item[2]})\n"
            tg_send(msg)

    elif text == "/platforms":
        tg_send(f"📢 Platforms:\n"
                f"Telegram: {'✅' if TELEGRAM_CHANNEL_ID else '❌'}\n"
                f"Discord: {'✅' if DISCORD_WEBHOOK else '❌'}\n"
                f"Bluesky: {'✅' if BLUESKY_HANDLE else '❌'}\n"
                f"Mastodon: {'✅' if MASTODON_TOKEN else '❌'}\n"
                f"Telegraph: ✅\n"
                f"Beehiiv: {'✅' if BEEHIIV_API_KEY else '❌'}\n"
                f"FOREX: ✅")
    ...
    # NEW: Auto-reply to any other message
    else:
        tg_send("💭 Thinking...")
        reply = ai_ask(
            f"You are NOH4Q, an AI agent running on Telegram. "
            f"Reply helpfully and concisely (under 200 chars) to: {text}"
        )
        tg_send(reply[:500])
...
def work_loop():
    ...
    while True:
        try:
            ...
            if queued:
                topic = queued["topic"]
                logger.info(f"📝 Processing queued topic: {topic}")
            else:
            ...
            # 9. Weekly report (every 42 cycles = ~7 days)
            if cycle_count % 42 == 0:
                summary = db.get_analytics_summary()
                msg = f"📊 WEEKLY REPORT\n━━━━━━━━━━━━━━━━━━━\n"
                for s in summary:
...
# START
# ============================================
logger.info("🚀 NOH4Q Phase 4D starting...")
threading.Thread(target=tg_poll, daemon=True).start()
threading.Thread(target=work_loop, daemon=True).start()
logger.info("✅ Analytics + Auto-Reply + Queue enabled")
